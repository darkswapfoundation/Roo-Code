...
import { LLMInfo, LLMInstanceInfo } from "@lmstudio/lms-shared-types"
	...
	try {
		...
		try {
			...
			for (const model of downloadedModels) {
				...
				models[model.path] = parseLMStudioModel(model as LLMInfo)
			}
		} catch (error) {
		...
		const loadedModels = (await client.llm.listLoaded()) as Array<LLMInstanceInfo>
