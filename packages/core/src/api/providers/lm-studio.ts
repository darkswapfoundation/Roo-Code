...
export class LmStudioHandler extends BaseProvider implements SingleCompletionHandler {
	...
	override async *createMessage(
		...
		let inputTokens = 0
		try {
			const contentForTokenCounting: Anthropic.Messages.TextBlockParam[] = [{ type: "text", text: systemPrompt }]
			for (const msg of messages) {
				if (typeof msg.content === "string") {
					contentForTokenCounting.push({ type: "text", text: msg.content })
				} else {
					for (const part of msg.content) {
						if (part.type === "text") {
							contentForTokenCounting.push({ type: "text", text: part.text })
						}
					}
				}
			}
			inputTokens = await this.countTokens(contentForTokenCounting)
		} catch (err) {
