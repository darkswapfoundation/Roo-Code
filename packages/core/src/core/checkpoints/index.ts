...
export async function getCheckpointService(
	...
	try {
		...
		log(`[Task#getCheckpointService] ${(err as Error).message}`)
		task.enableCheckpoints = false
...
async function checkGitInstallation(
	...
	try {
		...
		try {
			...
			log(`[Task#getCheckpointService] initShadowGit -> ${(err as Error).message}`)
			task.enableCheckpoints = false
		}
	} catch (err) {
		log(`[Task#getCheckpointService] Unexpected error during Git check: ${(err as Error).message}`)
...
export async function checkpointDiff(task: Task, { commitHash, mode }: CheckpointDiffOptions) {
	const service = await getCheckpointService(task)
