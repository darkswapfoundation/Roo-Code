...
export async function parseMentions(
	...
	for (const [commandName, command] of validCommands) {
		try {
			...
			parsedText += `\n\n<command name="${commandName}">\nError loading command '${commandName}': ${
				error instanceof Error ? error.message : error
			}\n</command>`
		}
	}
	...
	try {
		...
		if (stats.isFile()) {
			...
			try {
				...
				return `(Failed to read contents of ${mentionPath}): ${error instanceof Error ? error.message : error}`
			}
		...
		throw new Error(`Failed to access path "${mentionPath}": ${error instanceof Error ? error.message : error}`)
	}
}
